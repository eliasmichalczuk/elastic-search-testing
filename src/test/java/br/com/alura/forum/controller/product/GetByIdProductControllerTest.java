...
public class GetByIdProductControllerTest {
	...
	void shouldReturnProduct1() throws Exception {
		...
		mockMvc.perform(get(PATH + "/1").contentType(APPLICATION_JSON_VALUE)).andExpect(status().is2xxSuccessful())
				.andExpect(jsonPath("$.id", is(1))).andExpect(jsonPath("$.name", is("produto 1")));
	}
